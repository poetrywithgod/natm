...
export interface Form2Draft {
  ...
  protocolNotes: Record<string, unknown>;
...
export async function fetchOrCreateForm2Draft(
  ...
  const { data: created, error: createError } = await supabase
    .from("form2_submissions")
    .insert({ episode_id: episodeId, school_id: schoolId, student_id: studentId })
    .select("id")
    .single();
...
export async function submitForm2(form2Id: string, episodeId: string, submittedBy: string): Promise<void> {
  ...
  const { error: form2Error } = await supabase
    ...
    .eq("id", form2Id);
  if (form2Error) throw new Error(form2Error.message);

  const { error: episodeError } = await supabase
    .from("assessment_episodes")
    .update({ status: "form2_submitted", form2_submitted_at: now })
    .eq("id", episodeId);
