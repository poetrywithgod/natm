...
export default function SessionsTerms() {
  ...
  async function handleCreateTerm(sessionId: string, termNumber: number) {
    try {
      await createTerm(sessionId, termNumber);
      await loadAll();
    } catch (e) {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-4">
        ...
        {sessions.map((session) => {
          ...
          const existingTermNumbers = terms.map((t) => t.term_number);
              ...
              <div className="flex gap-2 flex-wrap">
                {[1, 2, 3].map((num) => {
                  ...
                  if (!term) {
                    return (
                      <button
                        ...
                        className="text-xs px-3 py-1.5 rounded border border-forest-700 text-forest-300 font-ui"
                      >
                        + Add Term {num}
                      </button>
                    );
                  ...
                  return (
                    <div
                      key={term.id}
                      className="flex items-center gap-2 text-xs px-3 py-1.5 rounded bg-forest-700 text-forest-100 font-ui"
                    >
                      ...
                      {term.is_current ? (
                        ...
                        <button
                          onClick={() => handleSetCurrentTerm(session.id, term.id)}
                          className="underline"
                        >
                          Set current
                        </button>
